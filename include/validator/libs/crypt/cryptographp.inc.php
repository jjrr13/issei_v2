...
if  ((!isset($_COOKIE['cryptcookietest'])) and ($_GET[$_GET['sn']]==""))
    ...
    readfile("{$httpCaptcha}/images/erreur3.png"); 
    exit;
    }
...
if ($_SESSION['cryptcptuse']>=$cryptusemax) {
   header("Content-type: image/png");
   readfile("{$httpCaptcha}/images/erreur1.png"); 
   exit;
   }
...
if ($delai < $cryptusetimer) { 
   switch ($cryptusertimererror) {
          case 2  : header("Content-type: image/png");
                    readfile("{$httpCaptcha}/images/erreur2.png"); 
                    exit;
...
for ($i=1;$i<= $charnb;$i++) {              
     ...
     $lafont="{$httpCaptcha}/fonts/".$tword[$i]['font'];
